...
void main() {
  testWidgets('E2E navigation flow Home -> Login -> Results -> Gallery -> Error', (WidgetTester tester) async {
  ...
  await tester.tap(find.byKey(const Key('login_submit')), warnIfMissed: false);
    ...
    if (find.textContaining('Welcome').evaluate().isEmpty) {
      ...
      await tester.pumpWidget(MaterialApp(
        home: ResultsPage(email: 'user@e2e.test', timestamp: DateTime.now().toIso8601String()),
        routes: {
          '/gallery': (ctx) => const GalleryPage(),
          '/error': (ctx) => const Scaffold(body: Center(child: Text('Error'))),
        },
      ));
      await tester.pumpAndSettle();
    }
