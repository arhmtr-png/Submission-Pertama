...
class RestaurantDetailPage extends StatefulWidget {
  ...
  const RestaurantDetailPage({super.key, required this.id, this.apiService});
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            Builder(
              builder: (context) {
                final detailProv = Provider.of<RestaurantDetailProvider>(
                  context,
                  listen: false,
                );
                ...
                try {
                  // listen:true so the Builder re-renders when favorites change
                  favProv = Provider.of<FavoriteProvider>(context);
                } catch (_) {
                  favProv = null;
                }
                ...
                // Use Provider's synchronous cache to avoid FutureBuilder here.
                return Consumer<FavoriteProvider>(
                  builder: (context, favProvConsumer, _) {
                    final isFav = favProvConsumer.isFavoriteSync(id);
                    final cs = Theme.of(context).colorScheme;
                    return Tooltip(
                      message: isFav
                      ...
                      child: IconButton(
                        ...
                        onPressed: () async {
                          ...
                          if (isFav) {
                            final ok = await favProvConsumer.removeFavorite(id);
                            if (!mounted) return;
                            ...
                            final ok = await favProvConsumer.addFavorite(r);
                            if (!mounted) return;
                            messenger.showSnackBar(
                              SnackBar(
                                content: Text(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Text(
                        ...
                        style: textTheme.bodyMedium?.copyWith(
                          // Improve contrast on dark theme: use onSurface with higher alpha
                          color: cs.onSurface.withAlpha(0xEE),
                          fontWeight: FontWeight.w600,
                        ),
                      ),
                    ),
